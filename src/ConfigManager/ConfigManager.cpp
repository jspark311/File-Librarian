...
char* ConfigManager::getConfigStringByKey(const char *desired_key){
    if (desired_key == NULL) return NULL;    // Bail-out clause....
    ConfigItem *cur    = this->current_config;
    while (cur != NULL) {
...
int ConfigManager::loadConfigFromDb(MySQLConnector *db){
    ...
    if (db->dbConnected()) {
        ...
        if (db->r_query("SELECT conf_key,conf_value FROM sv_configuration WHERE component IN ('CALLQLATOR', 'CALL_RECORDER', 'GLOBAL') ORDER BY FIELD(component, 'GLOBAL', 'CALL_RECORDER', 'CALLQLATOR') ASC;")) {
            MYSQL_ROW   row;
            MYSQL_RES   *result;
            result = db->result;
            if (result != NULL) {
